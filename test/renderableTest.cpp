...
public:
    ...
    void SetVertexDeclarationWithSection(
        Fire::VertexDeclarationSection const& section)
    {
        Fire::VertexDeclaration vDecl;
        vDecl.AddSection(
            section.m_attributeName, section.m_numElements, section.m_stride,
            section.m_offset);
...
{
bool operator==(
    Fire::VertexDeclarationSection const& lhs,
    Fire::VertexDeclarationSection const& rhs)
{
    return lhs.m_attributeName.compare(rhs.m_attributeName) == 0 &&
           lhs.m_numElements == rhs.m_numElements &&
           lhs.m_offset == rhs.m_offset && lhs.m_stride == rhs.m_stride;
}
} // namespace Fire
...
TEST_F(RenderableTest, AVertexDeclarationCanBeSetInRenderables)
{
    ...
    EXPECT_EQ(
        m_renderable.GetVertexDeclaration().GetSection(attributeName), section);
}
...
TEST_F(RenderableTest, SettingAVertexDeclarationOverridesThePreviousOne)
{
    ...
    Fire::VertexDeclarationSection const otherSection(
        attributeNameOther, 9, 21, 5);

    SetVertexDeclarationWithSection(section);
    SetVertexDeclarationWithSection(otherSection);

    EXPECT_EQ(
        m_renderable.GetVertexDeclaration().GetSection(attributeNameOther),
        otherSection);
}

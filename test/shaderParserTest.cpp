...
namespace
{
std::string const vertexShaderCode = "#version 440 \n"
                                     "uniform mat4 MVP;\n"
...
void CheckParsedInputVariable(
    Fire::ShaderParser::InputLayoutMap map, int layoutLocation,
    std::string const& name)
{
    ...
    auto const findIt = std::find_if(
        ...
        [&expectedInput](std::pair<int, std::string> input) {
            return input.first == expectedInput.first &&
                   input.second == expectedInput.second;
        });
...
TEST_F(ShaderParserTest, ParsesInputVariables)
{
    ...
    // CheckParsedInputVariable(inputs, 2, "color"); TODO: Regex parsing
    // einbauen
}
...
{
    auto const uniforms =
        m_parser.GetUniforms(); // TODO: Regex parsing einbauen
